...
export async function slowClick(element, driver, delay = 300) {
   try {
...
export async function getMakeYears(makeIndex) {
   ...
   try {
      ...
      await driver.sleep(3000) // 🛑 Pause to observe the page load
      ...
      await driver.quit()
      return yearOptions.map((elem) => elem.text)
   } catch (error) {
      console.log('Problem finding years.')
   }
}

export async function getTotalPaths(makeIndex) {
   let totalPaths = []
   ...
   try {
      ...
      for (let i = 0; i < yearOptions.length; i++) {
         ...
         for (let i = 0; i < modelOptions.length; i++) {
            ...
            // this is where you calc total
            totalPathsForYear += sizeOptions.length
         }
         totalPaths.push(totalPathsForYear)
      }
...
export async function selectDropdown(driver, dropdownId, optionToSelect) {
...
export async function getDropdownOptions(driver, selectID) {
   try {
...
export async function getBrandSrc(driver) {
   // Wait for the image element to be available
   let imgElement = await driver.findElement(By.css('img.img-responsive.ta-brand-image'))
...
export async function getTireTitle(driver) {
...
export async function getPricingData(driver) {
   let displayPrice = ''
...
export async function getFeaturesList(driver) {
...
export async function getBenefitsList(driver) {
...
export async function getSpecData(driver) {
...
export async function checkAndClickDealerButton(driver) {
   try {
...
export async function appendJsonToFile(newData) {
